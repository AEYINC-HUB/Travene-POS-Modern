...
public class TerminalConfigurationView extends ConfigurationView {
	...
	private IntegerTextField tfExtendDayHours;
	...
	private void initComponents() {
		...
		add(new JLabel(Messages.getString("TerminalConfigurationView.EXTENDDAYHOURS"))); //$NON-NLS-1$
		tfExtendDayHours = new IntegerTextField(3);
		
		add(tfExtendDayHours, "wrap"); //$NON-NLS-1$
	...
	public boolean save() {
		...
		int extendDayHours = tfExtendDayHours.getInteger();
		
		if ((extendDayHours < 0) || (extendDayHours>11)) {
			POSMessageDialog.showError(com.floreantpos.util.POSUtil.getFocusedWindow(), Messages.getString("TerminalConfigurationView.ERROR_RANGE_CHECK_EXTENDED_DAYS")); //$NON-NLS-1$
			return false;
		}
		...
		TerminalConfig.setExtendedDayHours(extendDayHours);
	...
	public void initialize() throws Exception {
		...
		tfExtendDayHours.setText(String.valueOf(TerminalConfig.getExtendedDayHours()));
